...
namespace Schemata
{
    public partial class ModelSet
    {
        public ModelSet(Model parent, params Schema[] schemata)
        {
            ...
            {
                Schema s = (Schema)Activator.CreateInstance(schema.ModelType, schema.Outline.SetItem("Parent", parent)).AssertNotNull();
                Models.Add(s.GetNewModel());
            }
        }
        ...
        public void Add(params Schema[] schemata)
        {
            ...
            foreach (Schema schema in schemata)
            {
                Model child = (Model)Activator.CreateInstance(schema.ModelType, schema.Outline.SetItem("Parent", Parent)).AssertNotNull();
                if (Models.Add(child))
                {
                    models.Add(child);
                }
        ...
        public void Add<TDirectory, TFile>(params string[] patterns) where TDirectory : Schema where TFile : Schema
        {
            ...
            foreach (string pattern in patterns)
            {
                ...
                {
                    Schema s = (Schema)Activator.CreateInstance(typeof(TDirectory), new ImmutableOutline.Builder { { "Name", Path.GetFileName(path) } }).AssertNotNull();
                    children.Add(s);
                }
                foreach (string path in Directory.GetFiles(Parent, pattern))
                {
                    Schema s = (Schema)Activator.CreateInstance(typeof(TFile), new ImmutableOutline.Builder { { "Name", Path.GetFileName(path) } }).AssertNotNull();
                    children.Add(s);
                }
            }
